...
public class Jsonfile {
	...
	public void save(ArrayList<Shape> shape) throws Exception {
		...
		for (int i = 0; i < shape.size(); i++) {
			...
			if (shape.get(i).getName() == "rectangle") {
			...
			}  else if (shape.get(i).getName() == "freehand") {
				...
				d += ((Line) shape.get(i)).getX1() + ","
						...
						+ ((Line) shape.get(i)).getColor().getRGB() + ","
						+ (((Line) shape.get(i)).isFilled());
				jGenerator.writeStringField("freehand", d);
	...
	public void load(String url, ArrayList<Shape> cshape, ArrayList<Shape> shape,
		...
		MouseClass paint = new MouseClass();
		paint.currentShapes =cshape;
		paint.shapes = shape;
		paint.ur.undo = undo;
		paint.ur.lastaction = lastaction;
		paint.repaint();
